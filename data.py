...
triples_to_take=40 ### change this to specify the number of triples that will be passed into the GNN

def transform_triple_to_hrt(triple,vocab_dic,relation_dic):
    """ Transforms triple-idx (as a whole) to h/r/t format """
    h, r, t = triple
    # print("##################### transform_triple_to_hrt #################")
    # print(vocab_dic[h])
    # print(relation_dic[r])

    return [vocab_dic[h], relation_dic[r], vocab_dic[t]]

def convert_tokens_to_id(kg,vocab_dic,relation_dic):
    kg = [transform_triple_to_hrt(triple,vocab_dic,relation_dic)  for triple in kg]
    return kg
...
class QAData(object):
    ...
    def __init__(self, logger, args, data_path, is_training):
        ...
        with open(self.data_path, "r") as f:
            ...
            cnt = 0
            invalid_lines = 0
            for ques in data_set:
                for q in ques['qas']:
                    question = f"{q['question']} \\n {ques['context']}"
                    answer = q["answers"]
                    triples = []
                    for i in range(triples_to_take):
                        if i >= len(q["kg_triplets"]):
                            triples.append(["_NAF_H", "_NAF_R", "_NAF_T"])
                        else:
                            triples.append(q["kg_triplets"][i])
                    self.data.append({
        ...
        preprocessed_path = os.path.join(
            ...
            self.data_path.split("/")[-1].replace(
                ...
                "{}{}-{}.json".format(
                    ...
                    postfix)))
        if self.load and os.path.exists(preprocessed_path) and False:
            ...
            kg_input_ids = []
            f1 = open("vocab_entities.json")
            vocab_ent = json.load(f1)
            f2 = open ("vocab_relation.json")
            vocab_relation = json.load(f2)
            for d in self.data:
                kg_input = []
                for trip in d["triples"]:
                    h, r, t = trip
                    kg_input.append([vocab_ent[h], vocab_relation[r] + 12549,vocab_ent[t]]) # 12549
                kg_input_ids.append(kg_input)

            self.dataset = MyQADataset(input_ids, attention_mask,
                                         decoder_input_ids, decoder_attention_mask, kg_input_ids,
...
class MyQADataset(Dataset):
    def __init__(self,
        ...
        self.kg_input_ids = torch.LongTensor(kg_input_ids)
    ...
    def __getitem__(self, idx):
        if not self.is_training:
            ...
            return self.input_ids[idx], self.attention_mask[idx], self.kg_input_ids[idx]
        ...
        return self.input_ids[in_idx], self.attention_mask[in_idx], \
            self.decoder_input_ids[out_idx], self.decoder_attention_mask[out_idx], self.kg_input_ids[in_idx]
