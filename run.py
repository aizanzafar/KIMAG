...
def run(args, logger):
    ...
    if args.do_train:
        ...
        if args.checkpoint is not None:
            model = CustomBart.from_pretrained("facebook/bart-large",
                                           state_dict=torch.load(args.checkpoint))
        else:
            model = CustomBart.from_pretrained("facebook/bart-large")

        #vocab_entities = json.load(open("vocab_entities.json", "r"))
        #vocab_relation = json.load(open("vocab_entities.json", "r"))
    ...
    for epoch in range(int(args.num_train_epochs)):
        for batch in train_data.dataloader:
            ...
            loss = model(input_ids=batch[0], attention_mask=batch[1],
                         decoder_input_ids=batch[2], decoder_attention_mask=batch[3],kg_input=batch[4],
                         is_training=True)
            #print("kg inputs", batch[4].size())
            if args.n_gpu > 1:
